"""
AWS Cognito utility functions for user pool management and authentication.

This module provides utilities for:
- Creating and managing AWS Cognito User Pools
- Generating secure passwords and storing them in Parameter Store
- Authenticating users and obtaining bearer tokens
- Setting up resource servers and machine-to-machine clients
- OAuth2 token management for client credentials flow
- IAM role creation for AgentCore services
- Lambda function deployment for gateway services

Functions:
    generate_secure_password: Generate cryptographically secure passwords
    get_or_generate_password: Retrieve or create passwords in Parameter Store
    get_password_from_parameter_store: Fetch stored passwords by client ID
    setup_cognito_user_pool: Complete Cognito User Pool setup with authentication
    reauthenticate_user: Re-authenticate existing users for new tokens
    get_or_create_user_pool: Find existing or create new user pools
    get_or_create_resource_server: Manage OAuth2 resource servers
    get_or_create_m2m_client: Handle machine-to-machine client setup
    get_token: Obtain OAuth2 tokens using client credentials
    create_agentcore_role: Create IAM roles for AgentCore agents
    create_agentcore_gateway_role: Create IAM roles for AgentCore gateways
    create_gateway_lambda: Deploy Lambda functions for gateway services
    delete_gateway: Remove gateway and associated targets
    delete_all_gateways: Clean up all existing gateways

This code has been adapted from:
    https://github.com/awslabs/amazon-bedrock-agentcore-samples/blob/main/01-tutorials/utils.py
"""
...
def generate_secure_password(length=16):
    ...
    Args:
        length (int): Length of password to generate. Defaults to 16 characters.
        
    Returns:
        str: Generated password containing mixed case letters, numbers and special characters.
    """
    alphabet = string.ascii_letters + string.digits + "!@#$%^&*"
    return ''.join(secrets.choice(alphabet) for _ in range(length))


def get_or_generate_password(pool_id, username):
    """Get password from Parameter Store or generate and store a new one.
    
    Args:
        pool_id (str): Cognito User Pool ID
        username (str): Username to get/generate password for
        
    Returns:
        str: Retrieved or newly generated password
        
    Note:
        Passwords are stored in Parameter Store with path: /cognito/{pool_id}/{username}/password
    """
    ...
    try:
        ...
        ssm.put_parameter(
            ...
            Value=password,
            Type='SecureString',
            Description=f'Password for Cognito user {username}'
        )
        return password
...
def get_password_from_parameter_store(client_id, username):
    ...
    Returns:
        str: Retrieved password from Parameter Store
...
def setup_cognito_user_pool():
    """Set up a new Cognito User Pool with an app client and test user.
    
    Creates a new Cognito User Pool with basic password policies, generates an app client,
    creates a test user with permanent credentials, and authenticates to obtain a bearer token.
    
    Returns:
        dict: Dictionary containing:
            - pool_id (str): ID of created Cognito User Pool
            - client_id (str): ID of created app client
            - bearer_token (str): Access token from successful authentication
            - discovery_url (str): OpenID Connect discovery URL for the User Pool
            
        Returns None if any errors occur during setup.
        
    Note:
        - Uses environment variable COGNITO_USERNAME (defaults to 'testuser') for test user
        - Stores user password securely in Parameter Store
        - Outputs key information to console during setup
    """
    ...
    try:
        ...
        # Generate or get credentials
        username = os.getenv('COGNITO_USERNAME', 'testuser')
        temp_password = generate_secure_password()
        permanent_password = get_or_generate_password(pool_id, username)
        ...
        print(f"Password stored in Parameter Store: /cognito/{pool_id}/{username}/password")
...
def reauthenticate_user(client_id):
    ...
    Note:
        Uses environment variable COGNITO_USERNAME (defaults to 'testuser') 
        to identify which user to re-authenticate
    """
    boto_session = Session()
    region = boto_session.region_name
    # Initialize Cognito client
...
def get_or_create_user_pool(cognito, USER_POOL_NAME):
    """Get an existing Cognito user pool or create a new one.
    
    Args:
        cognito: Boto3 Cognito client instance
        USER_POOL_NAME (str): Name of the user pool to find/create
        
    Returns:
        str: ID of the existing or newly created user pool
        
    Note:
        - Searches for existing pool by name
        - If found, retrieves and prints domain info
        - If not found, creates new pool and domain
        - Domain name is created from user pool ID (lowercase, no underscores)
    """
...
def get_or_create_resource_server(cognito, user_pool_id, RESOURCE_SERVER_ID, RESOURCE_SERVER_NAME, SCOPES):
    """Get an existing resource server or create a new one for a Cognito user pool.
    
    Args:
        cognito: Boto3 Cognito client instance
        user_pool_id (str): ID of the Cognito user pool
        RESOURCE_SERVER_ID (str): Identifier for the resource server
        RESOURCE_SERVER_NAME (str): Display name for the resource server
        SCOPES (list): List of OAuth scopes to configure for the resource server
        
    Returns:
        str: Identifier of the existing or newly created resource server
        
    Note:
        Creates a new resource server with the specified scopes if one does not exist
    """
    try:
...
def get_or_create_m2m_client(cognito, user_pool_id, CLIENT_NAME, RESOURCE_SERVER_ID):
    """Get an existing machine-to-machine client or create a new one for a Cognito user pool.
    
    Args:
        cognito: Boto3 Cognito client instance
        user_pool_id (str): ID of the Cognito user pool
        CLIENT_NAME (str): Name for the client application
        RESOURCE_SERVER_ID (str): ID of the resource server to grant access to
        
    Returns:
        tuple: A tuple containing:
            - str: Client ID of the existing or newly created client
            - str: Client secret for the client
            
    Note:
        Creates a new OAuth2 client with client credentials flow if one does not exist,
        configured with gateway read/write scopes for the specified resource server.
    """
...
def get_token(user_pool_id: str, client_id: str, client_secret: str, scope_string: str, REGION: str) -> dict:
    ...
    Makes a request to the Cognito token endpoint to obtain an access token using
    ...
    Args:
        ...
        scope_string (str): Space-separated list of requested OAuth scopes
    ...
    Note:
        The user pool domain is derived from the user pool ID by removing underscores.
    """
    try:
...
def create_agentcore_role(agent_name):
    """Create or recreate an IAM role for an Amazon Bedrock Agent Core agent.

    Creates an IAM role with permissions required for Agent Core functionality including:
    ...
    - Agent Core API access
    - Workload identity token management

    If the role already exists, it will be deleted and recreated with updated permissions.

    Args:
        agent_name (str): Name of the agent to create the role for. Used in role name and resource ARNs.

    Returns:
        dict: Response from IAM create_role API containing the created role details including:
            - Role ARN
            - Role ID
            - Role name
            - Creation date
    ...
    Raises:
        ClientError: If there are errors interacting with IAM APIs
        Exception: For other errors during role or policy creation
    """
